...
class Viewport extends JPanel implements ActionListener {
  ...
  enum ID { EXIT, FULLSCREEN, MINIMIZE, RESIZE,
            CTRL, ALT, MENUKEY, CTRLALTDEL,
  ...
  public void handleKeyPress(int keyCode, int keySym)
  {
    ...
    if (VncViewer.os.startsWith("windows")) {
      ...
      if (downKeySym.containsValue(XK_Control_L) &&
        ...
        try {
          cc.writer().keyEvent(XK_Control_L, 0x1d, false);
          cc.writer().keyEvent(XK_Alt_R, 0xb8, false);
        } catch (Exception e) {
          vlog.error("%s", e.getMessage());
          //exit_vncviewer(e.getMessage());
          System.exit(1);
        }
      }
    }
    ...
    if (VncViewer.os.startsWith("windows")) {
      ...
      if (downKeySym.containsValue(XK_Control_L) &&
        ...
        try {
          cc.writer().keyEvent(XK_Control_L, 0x1d, true);
          cc.writer().keyEvent(XK_Alt_R, 0xb8, true);
        } catch (Exception e) {
          vlog.error("%s", e.getMessage());
          //exit_vncviewer(e.getMessage());
          System.exit(1);
  ...
  public void handleKeyRelease(int keyCode)
  {
    ...
    try {
      if (keyCode > 0xffff)
        cc.writer().keyEvent(iter, 0, false);
      else
        cc.writer().keyEvent(iter, keyCode, false);
    } catch (Exception e) {
      vlog.error("%s", e.getMessage());
      //exit_vncviewer(e.getMessage());
      System.exit(1);
  ...
  private int handleSystemEvent(AWTEvent event)
  {
    ...
    if (event instanceof KeyEvent) {
      ...
      if (ev.getID() == KeyEvent.KEY_PRESSED) {
        ...
        try {
        keyCode = 
          KeyMap.code_map_java_to_qnum.get(ev.getKeyCode())[ev.getKeyLocation()];
        } catch(java.lang.NullPointerException e) {
          vlog.info(ev.paramString());
        }
